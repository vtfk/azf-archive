...
module.exports = async (personData, upn, manager, requireAccessGroups) => {
  ...
  if (archiveManagerRes.length === 0) {
    ...
    const mailStr = `Halla!<br><br>Arkiveringsroboten kunne ikke finne 360-bruker for leder "${displayName}" (${onPremisesUserPrincipalName}).<br>Brukeren ${displayName} jobber i ${department} - ${companyName}.<br><br>Kunne dere ha opprettet bruker for lederen, slik at arkiveringsroboten får arkivert? 😎<br>`
    await sendmail({

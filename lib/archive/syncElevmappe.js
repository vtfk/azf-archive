...
module.exports = async personData => {
  ...
  if (elevmappeRes.length >= 1 && elevmappeRes[0].CaseNumber) {
    ...
    console.log(elevmappeRes[0].Contacts[0].Address.StreetAddress, streetAddress)
    ...
    if (needsUpdate) { // PrivatePerson was updated, update elevmappe as well
      logger('info', ['syncElevmappe', 'Elevmappe metadata is different from person info (name, ssn, streetAddress), or has wrong case-metadata (title, unofficialTitle)), will update to match person info and case-metadata'])
      if (updatedSsn) {
      ...
      logger('info', ['syncElevmappe', 'PrivatePerson was not updated, and elevmappe-metadata was correct, no need to update elevmappe'])

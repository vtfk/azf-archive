...
module.exports = async personData => {
  ...
  if (addressCode === 6 || addressCode === 7) {
    ...
    if (privatePersonRes.length === 1 && privatePersonRes.PersonalIdNumber) {
    ...
    if (privatePersonRes.length === 0) {
      ...
      logger('info', ['syncPrivatePerson', `Found address block both in DSF but not privatePerson in P360. Created PrivatePerson with "Recno: ${person.recno}" and sent mail to address block-contact`])
    } else {
      // Send mail til 360 administrator med feilmld
  ...
  // Hvis nytt fnr - sjekk om det finnes privatperson på det gamle og det nye - hvis begge eksisterer, send mail til arkiv om at det må ryddes. Hvis bare det eksisterer privatperson på det gamle, oppdater denne med nytt fnr. Hvis det ikke eksisterer noen, opprett ny privatperson med det nye fnr. Hvis det bare eksisterer på det nye - wtf?? Gjør som vanlig tipper jeg.
  ...
  if (ssn !== oldSsn) {
    flow.oldPrivatePersonRes = await callTemplateData('elevmappe', 'get-private-person', { oldSsn })
  ...
  if (!flow.hasOldPrivatePerson && !flow.hasPrivatePerson) { // Har ingen privatperson i P360, hverken på gammelt eller nytt fnr
    ...
    // Send mail til arkivarer om at det må ryddes opp i privatpersonene med fnr blablabla
  ...
  // TODO: Legge inn e-post varsel på alt mulig rart (søk på "send mail")
